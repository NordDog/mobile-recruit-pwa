<template>
  <v-container>
    ...
    <v-sheet elevation="1" class="pa-2">
          ...
          <v-expansion-panel>
            ...
            <v-expansion-panel-content style="text-align: left" class="pa-2">
              ...
              <div class="d-flex justify-center pa-5">
                <v-btn color="primary" v-if="is_su > 0" @click="gotoPos()"
                  >Изменить требования</v-btn
                >
              </div>
            ...
            <v-expansion-panel-content>
              <div class="d-flex pa-2">
                ...
                <v-btn icon @click="filterDial = true">
                  <v-icon large color="grey darken-1"> mdi-filter </v-icon>
                  <!--                <img class="filterimg" src="../../public/filter.png" alt="">-->
                </v-btn>
              </div>
              <v-simple-table>
                <tbody>
                  <tr v-for="asp in filteredAsps" :key="asp.ID">
                    <td>{{ asp.FULL_NAME }}</td>
                    <td>
                      <a :href="'tel:' + asp.CONT_PHONE">
                        <v-icon large color="grey darken-2"> mdi-phone </v-icon>
                      </a>
                    </td>
                    <td style="width: 50%">
                      <v-select
                        @change="changeStatus(asp.ID, $event)"
                        :items="statuses"
                        item-text="NAME"
                        item-value="STATUS_ID"
                        v-model="asp.STAGE_ID"
                        solo
          ...
          <v-expansion-panel>
            ...
            <v-expansion-panel-content class="pa-2">
              ...
              <v-simple-table class="mt-2 mb-2">
                <tbody>
                  <tr v-for="empl in filteredEmpls" :key="empl.ID">
                    <td>{{ empl.FULL_NAME }}</td>
                    <td>
                      <a :href="'tel:' + empl.CONT_PHONE">
                        <v-icon large color="grey darken-2"> mdi-phone </v-icon>
                      </a>
                    </td>
                    <td>
                      <v-select
                        @change="changeStatus(empl.ID, $event)"
                        :items="statuses"
                        item-text="NAME"
                        item-value="STATUS_ID"
                        v-model="empl.STAGE_ID"
                        solo
      ...
      <div
        v-if="is_su > 0"
        class="d-flex column justify-center pa-5 newAppButtonsDiv"
      >
        <v-btn @click="archiver()" color="primary" class="mybtn newPosBtn"
          >Убрать в архив</v-btn
        >
      </div>
      <v-dialog v-model="rejectDial" width="300">
        <dialog-content
          v-if="rejectDial"
          :id="emplToDial"
          :status="statusToDial"
      ...
      <v-dialog v-model="filterDial" width="340">
        <v-card>
          <v-card-title>
            <p
              style="margin-right: auto"
              class="text-subtitle2 text-sm-subtitle2"
            >
...
/*           
          if (obj.empls.length > 0) {
            for (let empl of obj.empls) {
              if (Array("422", "2000").includes(empl.CURR_STATUS))
                this.employers.push(empl);
              else if (!Array("429", "499", "2001").includes(empl.CURR_STATUS))
                this.aspirants.push(empl);
            }
          } 
          for( var key in obj){
            this.items.push({
 ...
 */
    ...
    getAspirantsList(){
      ...
      BX.rest.callMethod('recruit.aspirants.list', params, res=>{
        let list = res.data();
        this.aspirants = list.filter(item=>item.STAGE_SEMANTIC_ID == 'P');
        this.employers = list.filter(item=>item.STAGE_SEMANTIC_ID == 'S');
      });
    },
    getDealStages(){
    ...
    changeStatus(id, status){
      ...
      BX.rest.callMethod('recruit.aspirants.updatestatus', params, res=>{
  ...
  computed: {
    filteredAsps() {
      ...
      if (this.searchAsp.length > 1) {
        res = res.filter((asp) =>
          asp.FULL_NAME.toLowerCase().includes(this.searchAsp.toLowerCase())
        );
      }
    ...
    filteredEmpls() {
      ...
      if (this.searchEmpl.length > 1) {
        res = res.filter((empl) =>
          empl.FULL_NAME.toLowerCase().includes(this.searchEmpl.toLowerCase())
        );
      }
      return res;
    },
