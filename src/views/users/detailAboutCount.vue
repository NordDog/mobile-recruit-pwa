        ...
        <v-sheet elevation="1" class="pa-2">
            <div class="fakeForm">
                <div class="d-flex justify-space-between mb-2">
                    ...
                    <p class="text-caption">{{dateInterview}}</p>
                    </div>
                    <div>
                       <v-btn  
                        color="primary"
                       elevation="2"
                        x-small @click="history = !history">История статусов</v-btn>     
                ...
                <v-text-field disabled label="Объект" outlined :value="getObjectName"></v-text-field>
                <v-text-field disabled label="Должность" outlined :value="position"></v-text-field>
                <v-select v-model="status" :items="statusItem" label="Статус" outlined></v-select>


                <v-menu ref="dateInterviewMenu" v-model="dateInterviewMenu" :close-on-content-click="false"
                    :return-value.sync="dateInterview" transition="scale-transition" offset-y min-width="auto">
                    <template v-slot:activator="{ on, attrs }">
                        <v-text-field v-model="dateInterview" label="Дата и время собеседования" outlined append-icon="mdi-calendar"
                    ...
                    <v-date-picker v-model="dateInter" no-title scrollable>
                        <div class="row">
                            <v-col class="col-12">
                                <p class="text-h6 text-center">Время собеседования</p>
                        ...
                        <v-col class="col-12">
                        <v-spacer></v-spacer>
                        <v-btn text color="primary" @click="dateInterviewMenu = false">
                            Cancel
                        </v-btn>
                        <v-btn text color="primary" @click="dateTimeinner(), $refs.dateInterviewMenu.save(dateInterview)"><!--$refs.dateInterviewMenu.save(dateInterview)-->
                            OK
                        </v-btn>
                        </v-col>
                        </div>
                ...
                <v-select  v-model="okSb" :items="okSbItems" label="Прошел Сб" outlined></v-select>
                <!--SB end-->
                ...
                <v-menu ref="dateOnLkmMenu" v-model="dateOnLkmMenu" :close-on-content-click="false"
                    ...
                    <template v-slot:activator="{ on, attrs }">
                        <v-text-field v-model="dateOnLkm" label="Дата отправки ЛКМ" outlined append-icon="mdi-calendar"
                            readonly v-bind="attrs" v-on="on"></v-text-field>
                    </template>
                ...
                <v-menu ref="dateOffLkmMenu" v-model="dateOffLkmMenu" :close-on-content-click="false"
                    ...
                    <template v-slot:activator="{ on, attrs }">
                        <v-text-field v-model="dateOffLkm" label="Дата прохождения ЛКМ" outlined
                            append-icon="mdi-calendar" readonly v-bind="attrs" v-on="on"></v-text-field>
                    </template>
                ...
                <!--Паспорт-->
                <v-text-field v-model="serialPassport" label="Серия паспорта" outlined></v-text-field>
                <v-text-field v-model="numberPassport" label="Номер паспорта" outlined></v-text-field>
                <v-text-field v-model="issuedBy" label="Кем выдан" outlined></v-text-field>
              <v-menu ref="issuedDateMenu" v-model="issuedDateMenu" :close-on-content-click="false"
                    :return-value.sync="issuedDate" transition="scale-transition" offset-y min-width="auto">
                    <template v-slot:activator="{ on, attrs }">
                        <v-text-field v-model="issuedDate" label="Дата выдачи паспорта" outlined append-icon="mdi-calendar"
                            readonly v-bind="attrs" v-on="on"></v-text-field>
                    </template>
                    <v-date-picker v-model="issuedDate" no-title scrollable>
                        ...
                        <v-btn text color="primary" @click="$refs.issuedDateMenu.save(issuedDate)">
                            OK
                        </v-btn>
                    </v-date-picker>
                </v-menu>
                ...
                <v-text-field v-model="registrationAddress" label="Адрес регистрации" outlined></v-text-field>

             <v-text-field v-model="addressOfActualResidence" label="Адрес фактического пребывания" outlined></v-text-field>
            ...
            ></v-file-input>
             ...
             <v-expansion-panels focusable class="mt-2 mb-3">
                <v-expansion-panel>
                <v-expansion-panel-header>Сканы документов</v-expansion-panel-header>
             ...
             <v-text-field v-model="INN" label="ИНН" outlined></v-text-field>


                <v-text-field v-model="comment" label="Коментарий" outlined></v-text-field>
...
export default {
    ...
    methods: {
         ...
         dateTimeinner(){
            this.dateInterview = this.dateInter+' '+this.hours+':'+this.minutes
        },
            ...
            await axios({
                ...
                .then((response) => {
                    ...
                    obj.sex.forEach(item => {
                        ...
                        this.object = obj.cont.UF_CRM_1625539465891> 0 ?obj.cont.UF_CRM_1625539465891 : this.getObjectId
                        this.title = obj.req.length > 0 && obj.req.RQ_LAST_NAME.length > 0 && obj.req.RQ_FIRST_NAME.length >0 ?obj.req.RQ_LAST_NAME+" "+ obj.req.RQ_FIRST_NAME : obj.cont.LAST_NAME + ' ' + obj.cont.NAME
                        this.name =  obj.req.length > 0 && obj.req.RQ_FIRST_NAME.length > 0 ? obj.req.RQ_FIRST_NAME : obj.cont.NAME
                        this.last_name =obj.req.length > 0 &&  obj.req.RQ_LAST_NAME.length > 0 ? obj.req.RQ_LAST_NAME : obj.cont.LAST_NAME
                        this.second_name = obj.req.length > 0 && obj.req.RQ_SECOND_NAME.length > 0 ? obj.req.RQ_SECOND_NAME : obj.cont.SECOND_NAME
                       ...
                       if(obj.req.addr) {
                            obj.req.addr.forEach(item=>{
                                ...
                                if(item.TYPE_ID == 4){
                                    this.registrationAddress = item.ADDRESS_1 ? item.ADDRESS_1 : item.ADDRESS_2
                                }

                                if(item.TYPE_ID == 1){
                                    this.addressOfActualResidence = item.ADDRESS_1 ? item.ADDRESS_1 : item.ADDRESS_2
                                }
            

                            })
                       }
                            console.log(obj.cont.img)
                        if(obj.cont.img){
